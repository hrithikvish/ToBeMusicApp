...
import com.hrithikvish.apitask2.util.Utils.Companion.largeLog
...
class MainActivity : AppCompatActivity() {
    ...
    private fun handleGetStreamsBtnClick(streamType: StreamType) {
        binding.youtubeVideoName.text = ""
        val adapter = binding.streamsRV.adapter as? StreamingUrlsRVAdapter
        adapter?.upsertAudioStreamsList(emptyList(), emptyList(), StreamType.NULL)
    ...
    private fun getVideoInfo(youtubeUrl: String, streamType: StreamType) {
        ...
        try {
            ...
            e.printStackTrace()
            runOnUiThread {
                Snackbar.make(binding.root, "Check your internet connectivity", Toast.LENGTH_SHORT).show()
                hideLoadingBar()
            }
            return
        }
            ...
            e.printStackTrace()
            runOnUiThread {
                Snackbar.make(binding.root, "Something went wrong, try again", Toast.LENGTH_SHORT).show()
                hideLoadingBar()
            }
            return
        }
        ...
        videoStreams.sortBy { it.getResolution() }

        val subtitleStream = extractor.getSubtitles(MediaFormat.TRANSCRIPT1).filter {
            !it.isAutoGenerated && it.languageTag == "en"
        }
        ...
        runOnUiThread {
            ...
            when (streamType) {
                StreamType.AUDIO -> adapter.upsertAudioStreamsList(sortedAudioStreams, subtitleStream, StreamType.AUDIO)
                StreamType.VIDEO -> adapter.upsertAudioStreamsList(videoStreams, streamType = StreamType.VIDEO)
                else -> {}
            }
